# ghcp-learning-lab
Learning GitHub Copilot, Git, and AI-powered development workflows.

## Learning goals

- Understand Git and GitHub fundamentals
- Practice working with branches and pull requests
- Explore GitHub Copilot for product design workflows

## Git vs GitHub: A Figma Analogy

**Git** is like Figma's local autosave—it tracks changes to your files on your computer, creating a history you can revert to anytime.

**GitHub** is like Figma's cloud collaboration—it's where you upload your project so others can see it, contribute, and sync changes. Git handles the *how* (version control), GitHub handles the *where* (cloud storage + collaboration).

Think: Git = your design edits, GitHub = sharing your file link with teammates.
